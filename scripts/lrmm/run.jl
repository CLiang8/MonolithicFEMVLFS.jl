...
# # Production Low_freq run
# @with_kw struct run_params_low
#   name = resDir*"/Low_freq"
#   order::Int = 2
...
# end
# # Production High_freq run
# @with_kw struct run_params_high
#   name = resDir*"/High_freq"
#   order::Int = 2
#   vtk_output::Bool = true
...
# params = run_params_low()
# Memb_undamped_2D.main(params)
# params = run_params_high()
# Memb_undamped_2D.main(params)

# trial run for a different resonator natural frequency
...
# 文件路径
low_path  = "data/sims_mem_freq_lrmm/Low_freq/mem_data.jld2"
high_path = "data/sims_mem_freq_lrmm/High_freq/mem_data.jld2"
merged_path = "data/sims_mem_freq_lrmm/Coefficients/mem_data_merged.jld2"

# 加载
data_low  = load(low_path)
data_high = load(high_path)

# 初始化结果字典
data_merged = Dict{String, Any}()

# 拼接所有键对应的值
for key in keys(data_low)
    val_low = data_low[key]
    val_high = data_high[key]

    if isa(val_low, DataFrame) && size(val_low, 2) == size(val_high, 2)
        data_merged[key] = vcat(val_low, val_high)
    elseif isa(val_low, DataFrame)
        @warn "Skipping key $key due to column mismatch: $(size(val_low)) vs $(size(val_high))"
    elseif isa(val_low, AbstractVector)
        data_merged[key] = vcat(val_low, val_high)
    else
        error("Unhandled or incompatible type for key $key")
    end
end

wsave(merged_path, data_merged)
println("✅ 合并完成，已保存为: $merged_path")
end
