...
rM = [1.0e3,1.0e3,1.0e3,1.0e3] #Kg
rω_list = [1.5, 2.5, 3.5, 4.5] #rad/s
rK_list = rM.*rω_list.^2 #N/m
ζ = [0,0,0,0] # 0.05 #damping ratio
rC = 2*ζ.*sqrt.(rM .* rK_list) #N*s/m
println("Resonator Natural Frequency: ω1 = ", sqrt.(rK_list./rM), "rad/s")
...
V_Γqs = [ConstantFESpace(Ω, vector_type=Vector{ComplexF64}, field_type=VectorValue{1,ComplexF64}) for i in eachindex(rK_list)]
U_Γqs = [TrialFESpace(V) for V in V_Γqs]
î1 = VectorValue(1.0)
...
X = MultiFieldFESpace([U_Ω, U_Γκ, U_Γη, U_Γqs...])
Y = MultiFieldFESpace([V_Ω, V_Γκ, V_Γη, V_Γqs...])
...
if(diriFlag)
  ...
  a(u_trial, u_test) = begin
    ...
    # ∫( (ξ[1]⋅q[1])* 0.0 )dΩ  + 
    sum(
    (+im*ω*rC[i] - rK_list[i])/ρw * δ_p( v * ( (q[i]⋅î1) - η ) ) +
    (-rM[i]/ρw * ω^2) * δ_p(q[i]⋅ξ[i]) +
    (-im*ω*rC[i] + rK_list[i])/ρw * δ_p(q[i]⋅ξ[i] - (ξ[i]⋅î1)*η)
    for i in eachindex(rK_list)
  )
    end 
end
