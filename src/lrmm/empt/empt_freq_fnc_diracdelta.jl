...
function run_freq(ω, A, α)
  ...
  ρw = 1025 # kg/m3 water
  f=ρw * ω^2 * A # Force amplitude

  println("ω = $ω, T = $T, λ = $λ")

  # ηᵢₙ(x) = η₀*exp(im*k*x[1] + im*α)
  # ηᵢₙ(x) = A*exp(im*k*(x[1]-x₀) + im*α)*exp(-((x[2]-z₀)^2)/0.5^2) #trial for later with damping zone
  ...
  δp = DiracDelta(Ω, Point(x₀, z₀))  # 内部点源

  a((ϕ, κ), (w, u)) =
    ∫( ∇(w) ⋅ ∇(ϕ) )dΩ +
    ∫( im*ω*w*κ )dΓfs +
    ∫( βₕ*(u + αₕ*w)*(g*κ - im*ω*ϕ) )dΓfs # no damping，bounded water domain
...
mesh_ry = 1.1 #Ratio for Geometric progression of eleSize
xₒ = 0.0
domain =  (xₒ, xₒ+LΩ, -H0, 0.0) #将原文中x0 改为了 xo，x0是指点源位置
partition = (nx, ny)
@show LΩ
@show domain
@show partition
