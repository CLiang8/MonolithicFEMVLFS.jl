...
# # -------------------Solution 1 natural freq of mem-------------------
# tick()

# # transfer M44 to K Matrix
...
# # -------------------Solution 2 natural freq of lrmm-------------------
# tick()

# # transfer M11 to K Matrix
# C14
# K_1R = - C41 * ((-ω^2 * M11 + K11 + K11r) \ Matrix(C14))
# Ktot = K44 + K_1R


# tock()

# λ = LinearAlgebra.eigvals(M44\Matrix(Ktot))
# V = LinearAlgebra.eigvecs(M44\Matrix(Ktot))
# @show sum(imag.(λ))
# ωₙ = real.(λ)
# ωₙ = ifelse.(ωₙ .<0, 0, ωₙ)
# @show ωₙ = sqrt.(ωₙ)
...
# wsave(filename*"_modesdata2.jld2", data)

# -----------------Solve eigenproblem another way-------------------
...
tick()
λ, V = eigen(Matrix(B) \ Matrix(A))   # or use eigen(A, B)
tock()
...
N = 6
plt = plot(layout = (N, 1), size = (800, 250 * N), legend = false)
...
savefig(plt, filename * "_modes_dry_lrmm.png")
display(plt)
end

barplot_q = bar(abs.(q_modes[1, 2:10]), xlabel="Mode number", ylabel="|q|", title="Resonator modal participation (first 10 modes)")
savefig(barplot_q, filename * "_q_participation.png")
